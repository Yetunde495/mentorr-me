...
export default function FlipStack({
  ...
  const getCardVariants = (index: number) => {
    ...
    return {
      ...
      animate: {
        x: positionFromCenter * 100,
        y: Math.abs(positionFromCenter) * 30,
        rotate: positionFromCenter * 20,
        scale: 1,
              ...
              >
                {cards.map((card, index: number) => {
                  ...
                  return (
                    <motion.div
                      key={card.id}
                      className="absolute origin-bottom px-4"
                      initial="initial"
                      animate={isInView ? "animate" : "initial"}
                      variants={variants}
                      transition={{
                    ...
                    >
                      <Card className="w-full max-md:h-80 h-75  shadow-2xl border-0 bg-white dark:bg-gray-800 overflow-hidden">
